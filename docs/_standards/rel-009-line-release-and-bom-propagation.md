...
`REL-009.CHECK-005` reads the declaration against GitHub. `repository` and every key of `repositories` name a repository in the provider's organization that GitHub records as a fork, named `patch-` followed by its parent's name as [FORK-001]({{ site.baseurl }}/standards/fork-001-repository-naming/) requires. Every member is released from exactly one of them. Where a member's own POM states a GitHub address for itself, that address is the parent of the repository the declaration names for it, so a declaration cannot move an artifact away from the project that ships it. A member that states none is placed by the declaration alone, which is the case this standard exists for.

Only an address the member states for itself governs, and only where it names a repository and stops there. An address a member inherits from a parent POM is the parent project's statement, not the member's; and Maven derives a module's `url` and `scm` from its parent's by appending the module's `artifactId`, so an address carrying a path under a repository is that derivation rather than a statement, whether the module leaves it to be resolved or a flattened POM publishes the resolved value in the module's own `<scm>`. `org.springframework.data:spring-data-r2dbc` is the shape: its POM on Central says `https://github.com/spring-projects/spring-data-build/spring-data-parent/spring-data-r2dbc`, two module segments hung off the address `spring-data-parent` inherits from `spring-data-build`, while the module's source is in `spring-projects/spring-data-r2dbc` and `spring-data-build` ships none of it. Read as the module's own address it refuses the declaration that places the module correctly and leaves no repository the module could be released from at all. The exchange takes the repository for an uploaded artifact from the declaration that lists it.
